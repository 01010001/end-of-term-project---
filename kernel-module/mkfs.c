...
struct superblock {
    union {
        struct vcfs_sb_info info;
        char padding[vcfs_BLOCK_SIZE]; /* Padding to match block size */
    };
};

_Static_assert(sizeof(struct superblock) == vcfs_BLOCK_SIZE);
...
static struct superblock *write_superblock(int fd, struct stat *fstats)
{
    ...
    uint32_t nr_blocks = fstats->st_size / vcfs_BLOCK_SIZE;
    uint32_t nr_inodes = nr_blocks;
    uint32_t mod = nr_inodes % vcfs_INODES_PER_BLOCK;
    if (mod)
        nr_inodes += vcfs_INODES_PER_BLOCK - mod;
    uint32_t nr_istore_blocks =
        DIV_ROUND_UP(nr_inodes, vcfs_INODES_PER_BLOCK);
    uint32_t nr_ifree_blocks = DIV_ROUND_UP(nr_inodes, vcfs_BLOCK_SIZE * 8);
    uint32_t nr_bfree_blocks = DIV_ROUND_UP(nr_blocks, vcfs_BLOCK_SIZE * 8);
    uint32_t nr_data_blocks =
        nr_blocks - nr_istore_blocks - nr_ifree_blocks - nr_bfree_blocks;

    memset(sb, 0, sizeof(struct superblock));
    sb->info = (struct vcfs_sb_info){
        .magic = htole32(vcfs_MAGIC),
...
static int write_inode_store(int fd, struct superblock *sb)
{
    ...
    char *block = malloc(vcfs_BLOCK_SIZE);
    if (!block)
        return -1;

    memset(block, 0, vcfs_BLOCK_SIZE);

    /* Root inode (inode 1) */
    struct vcfs_inode *inode = (struct vcfs_inode *) block;
    ...
    inode->i_uid = 0;
    inode->i_gid = 0;
    inode->i_size = htole32(vcfs_BLOCK_SIZE);
    ...
    int ret = write(fd, block, vcfs_BLOCK_SIZE);
    if (ret != vcfs_BLOCK_SIZE) {
        ret = -1;
        goto end;
    }
    ...
    for (i = 1; i < sb->info.nr_istore_blocks; i++) {
        ret = write(fd, block, vcfs_BLOCK_SIZE);
        if (ret != vcfs_BLOCK_SIZE) {
            ret = -1;
            goto end;
        }
...
static int write_ifree_blocks(int fd, struct superblock *sb)
{
    ...
    for (i = 1; i < le32toh(sb->info.nr_ifree_blocks); i++) {
        ret = write(fd, ifree, vcfs_BLOCK_SIZE);
        if (ret != vcfs_BLOCK_SIZE) {
            ret = -1;
            goto end;
        }
...
static int write_bfree_blocks(int fd, struct superblock *sb)
{
     ...
     */
    memset(bfree, 0xff, vcfs_BLOCK_SIZE);
    uint32_t i = 0;
    while (nr_used) {
    ...
    int ret = write(fd, bfree, vcfs_BLOCK_SIZE);
    if (ret != vcfs_BLOCK_SIZE) {
        ret = -1;
        goto end;
    }
    ...
    for (i = 1; i < le32toh(sb->info.nr_bfree_blocks); i++) {
        ret = write(fd, bfree, vcfs_BLOCK_SIZE);
        if (ret != vcfs_BLOCK_SIZE) {
            ret = -1;
            goto end;
        }
...
{
    char *buffer = calloc(1, vcfs_BLOCK_SIZE);
    if (!buffer) {
        perror("Failed to allocate memory");
        return -1;
    }

    ssize_t ret = write(fd, buffer, vcfs_BLOCK_SIZE);
    if (ret != vcfs_BLOCK_SIZE) {
...
int main(int argc, char **argv)
{
    ...
    long int min_size = 100 * vcfs_BLOCK_SIZE;
