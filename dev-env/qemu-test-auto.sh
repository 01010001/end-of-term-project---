...
insmod vcfs.ko && echo -e "\033[1;32m[PASS]\033[0m" || { echo -e "\033[1;31m[FAIL]\033[0m"; poweroff -f; }

echo -n "[TEST] 2. Disk Formatting (128-byte Inode)... "
mkfs.vcfs /dev/sda > /dev/null 2>&1 && echo -e "\033[1;32m[PASS]\033[0m" || { echo -e "\033[1;31m[FAIL]\033[0m"; poweroff -f; }

echo -n "[TEST] 3. File System Mounting... "
mount -t vcfs /dev/sda /mnt && echo -e "\033[1;32m[PASS]\033[0m" || { echo -e "\033[1;31m[FAIL]\033[0m"; poweroff -f; }
...
echo "Attempting to restore inode $INODE"
vcfs restore $INODE
ls -la
if [ -f "test.txt" ]; then
    echo "File exists!"
    if [ "$(cat test.txt)" = "v0_data" ]; then 
        echo -e "\033[1;32m[PASS]\033[0m"
    else 
        echo -e "\033[1;31m[FAIL]\033[0m (Content mismatch)"
    fi
else 
    echo -e "\033[1;31m[FAIL]\033[0m (File not found after restore)"
fi
