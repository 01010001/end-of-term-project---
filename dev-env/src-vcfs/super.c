...
static struct kmem_cache *vcfs_inode_cache;
...
{
    vcfs_inode_cache = kmem_cache_create_usercopy(
        "vcfs_cache", sizeof(struct vcfs_inode_info), 0, 0, 0,
        sizeof(struct vcfs_inode_info), NULL);
    if (!vcfs_inode_cache)
        return -ENOMEM;
    return 0;
}
...
void vcfs_destroy_inode_cache(void)
{
    ...
    kmem_cache_destroy(vcfs_inode_cache);
}

static struct inode *vcfs_alloc_inode(struct super_block *sb)
{
    struct vcfs_inode_info *ci =
        kmem_cache_alloc(vcfs_inode_cache, GFP_KERNEL);
    if (!ci)
        return NULL;
...
static void vcfs_destroy_inode(struct inode *inode)
{
    struct vcfs_inode_info *ci = vcfs_INODE(inode);
    kmem_cache_free(vcfs_inode_cache, ci);
}

static int vcfs_write_inode(struct inode *inode,
                                struct writeback_control *wbc)
{
    struct vcfs_inode *disk_inode;
    struct vcfs_inode_info *ci = vcfs_INODE(inode);
    ...
    disk_inode = (struct vcfs_inode *) bh->b_data;
    ...
    /* Write VCFS Versioning metadata */
    disk_inode->version_id = cpu_to_le32(ci->version_id);
    disk_inode->version_timestamp = cpu_to_le32(ci->version_timestamp);
    disk_inode->prev_version_inode = cpu_to_le32(ci->prev_version_inode);
    disk_inode->is_deleted = cpu_to_le32(ci->is_deleted);
...
static void vcfs_put_super(struct super_block *sb)
{
    struct vcfs_sb_info *sbi = vcfs_SB(sb);
    int aborted = 0;
    int err;
...
static int vcfs_sync_fs(struct super_block *sb, int wait)
{
    struct vcfs_sb_info *sbi = vcfs_SB(sb);
    struct vcfs_sb_info *disk_sb;
    int i;
    ...
    disk_sb = (struct vcfs_sb_info *) bh->b_data;
    ...
    for (i = 0; i < sbi->nr_ifree_blocks; i++) {
        ...
        memcpy(bh->b_data, (void *) sbi->ifree_bitmap + i * vcfs_BLOCK_SIZE,
               vcfs_BLOCK_SIZE);
    ...
    for (i = 0; i < sbi->nr_bfree_blocks; i++) {
        ...
        memcpy(bh->b_data, (void *) sbi->bfree_bitmap + i * vcfs_BLOCK_SIZE,
               vcfs_BLOCK_SIZE);
...
static int vcfs_statfs(struct dentry *dentry, struct kstatfs *stat)
{
    struct super_block *sb = dentry->d_sb;
    struct vcfs_sb_info *sbi = vcfs_SB(sb);

    stat->f_type = vcfs_MAGIC;
    stat->f_bsize = vcfs_BLOCK_SIZE;
    ...
    stat->f_namelen = vcfs_FILENAME_LEN;
...
static journal_t *vcfs_get_dev_journal(struct super_block *sb,
                                           dev_t journal_dev)
{
    struct vcfs_sb_info *sbi = vcfs_SB(sb);
    struct buffer_head *bh;
    ...
    sb_block = vcfs_BLOCK_SIZE / blocksize;
    offset = vcfs_BLOCK_SIZE % blocksize;
     ...
     * len = CapacityOfJournalDevice / vcfs_BLOCK_SIZE
     */
    ...
    journal = jbd2_journal_init_dev(bdev_handle->bdev, sb->s_bdev, start, len,
                                    sb->s_blocksize);
#elif vcfs_AT_LEAST(5, 15, 0)
    journal = jbd2_journal_init_dev(bdev, sb->s_bdev, start, len, blocksize);
#endif

    if (IS_ERR(journal)) {
        pr_err(
            "vcfs_get_dev_journal: failed to initialize journal, error "
            "%ld\n",
            PTR_ERR(journal));
...
static int vcfs_load_journal(struct super_block *sb,
                                 unsigned long journal_devnum)
{
    ...
    journal = vcfs_get_dev_journal(sb, journal_dev);
    if (IS_ERR(journal)) {
...
/* we use vcfs_OPT_JOURNAL_PATH case to load external journal device now */
#define vcfs_OPT_JOURNAL_DEV 1
#define vcfs_OPT_JOURNAL_PATH 2
static const match_table_t tokens = {
    {vcfs_OPT_JOURNAL_DEV, "journal_dev=%u"},
    {vcfs_OPT_JOURNAL_PATH, "journal_path=%s"},
};
static int vcfs_parse_options(struct super_block *sb, char *options)
{
    ...
    pr_info("vcfs_parse_options: parsing options '%s'\n", options);
    ...
    while ((p = strsep(&options, ","))) {
        ...
        switch (token) {
        case vcfs_OPT_JOURNAL_DEV:
            if (args->from && match_int(args, &arg)) {
                pr_err("vcfs_parse_options: match_int failed\n");
                return 1;
            }
            if ((ret = vcfs_load_journal(sb, arg))) {
                pr_err(
                    "vcfs_parse_options: vcfs_load_journal failed with "
                    "%d\n",
                    ret);
                return ret;
            }
        ...
        case vcfs_OPT_JOURNAL_PATH: {
            journal_path = match_strdup(&args[0]);
            if (!journal_path) {
                pr_err("vcfs_parse_options: match_strdup failed\n");
                return -ENOMEM;
            }
            ret = kern_path(journal_path, LOOKUP_FOLLOW, &path);
            if (ret) {
                pr_err(
                    "vcfs_parse_options: kern_path failed with error %d\n",
                    ret);
                kfree(journal_path);
                return ret;
            }
            ...
            if (S_ISBLK(journal_inode->i_mode)) {
                ...
                if ((ret = vcfs_load_journal(sb, journal_devnum))) {
                    pr_err(
                        "vcfs_parse_options: vcfs_load_journal failed "
                        "with %d\n",
                        ret);
                    return ret;
                }
...
static struct super_operations vcfs_super_ops = {
    .put_super = vcfs_put_super,
    .alloc_inode = vcfs_alloc_inode,
    .destroy_inode = vcfs_destroy_inode,
    .write_inode = vcfs_write_inode,
    .sync_fs = vcfs_sync_fs,
    .statfs = vcfs_statfs,
};
...
int vcfs_fill_super(struct super_block *sb, void *data, int silent)
{
    ...
    struct vcfs_sb_info *csb = NULL;
    struct vcfs_sb_info *sbi = NULL;
    struct inode *root_inode = NULL;
    int ret = 0, i;

    /* Initialize the superblock */
    sb->s_magic = vcfs_MAGIC;
    sb_set_blocksize(sb, vcfs_BLOCK_SIZE);
    sb->s_maxbytes = vcfs_MAX_FILESIZE;
    sb->s_op = &vcfs_super_ops;

    /* Read the superblock from disk */
    bh = sb_bread(sb, vcfs_SB_BLOCK_NR);
    if (!bh)
        return -EIO;

    csb = (struct vcfs_sb_info *) bh->b_data;
    ...
    /* Allocate sb_info */
    sbi = kzalloc(sizeof(struct vcfs_sb_info), GFP_KERNEL);
    if (!sbi) {
        ret = -ENOMEM;
        goto release;
    }
    ...
    sbi->ifree_bitmap =
        kzalloc(sbi->nr_ifree_blocks * vcfs_BLOCK_SIZE, GFP_KERNEL);
    if (!sbi->ifree_bitmap) {
        ret = -ENOMEM;
        goto free_sbi;
    }
    ...
    for (i = 0; i < sbi->nr_ifree_blocks; i++) {
        ...
        memcpy((void *) sbi->ifree_bitmap + i * vcfs_BLOCK_SIZE, bh->b_data,
               vcfs_BLOCK_SIZE);
    ...
    sbi->bfree_bitmap =
        kzalloc(sbi->nr_bfree_blocks * vcfs_BLOCK_SIZE, GFP_KERNEL);
    if (!sbi->bfree_bitmap) {
        ret = -ENOMEM;
        goto free_ifree;
    }
    ...
    for (i = 0; i < sbi->nr_bfree_blocks; i++) {
        ...
        memcpy((void *) sbi->bfree_bitmap + i * vcfs_BLOCK_SIZE, bh->b_data,
               vcfs_BLOCK_SIZE);
    ...
    ret = vcfs_parse_options(sb, data);
    if (ret) {
        pr_err("vcfs_fill_super: Failed to parse options, error code: %d\n",
               ret);
        return ret;
    }
