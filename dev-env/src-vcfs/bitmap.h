#ifndef vcfs_BITMAP_H
#define vcfs_BITMAP_H
 ...
 */
static inline uint32_t get_free_inode(struct vcfs_sb_info *sbi)
{
...
static inline uint32_t get_free_blocks(struct super_block *sb, uint32_t len)
{
    ...
    for (i = 0; i < len; i++) {
        ...
        memset(bh->b_data, 0, vcfs_BLOCK_SIZE);
...
static inline void put_inode(struct vcfs_sb_info *sbi, uint32_t ino)
{
...
static inline void put_blocks(struct vcfs_sb_info *sbi,
                              uint32_t bno,
                              uint32_t len)
{
